(function () {
	...
	/** @ngInject */
	function PrinterController($scope, $state, $mdDialog, $mdToast, api) {
		var vm = this;
		...
		vm.filter = {
			show: false,
			query: {
				fields: ['name', 'printerName', 'paperSize'],
				keyword: '',
				order: '',
				page: 1,
				limit: 10
		...
		vm.getPromise = function () {
			vm.promise = api.printer.get(vm.filter.query, vm.success).$promise;
		};
		...
		vm.destroy = function (item) {
			...
			$mdDialog.show(confirm).then(function () {
				api.printer.delete({id: item.id}, item, function () {
					$mdToast.show(
						$mdToast.simple()
